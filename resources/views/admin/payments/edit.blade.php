...
<div class="space-y-6 animate-fade-in">
    <div class="flex justify-between items-center border-b pb-3">
        <h1 class="text-2xl font-bold text-gray-800">Edit Transaksi Pembayaran #{{ $payment->id }}</h1>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6">
        <form action="{{ route('admin.payments.update', $payment->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $payment->email) }}" required
                ...
                <div>
                    ...
                    <div class="relative rounded-md shadow-sm">
                        ...
                        <input type="number" id="amount" name="amount" value="{{ old('amount', $payment->amount) }}" min="0" step="1" required
            ...
            <div>
                ...
                <div class="mb-3 space-y-2">
                    ...
                    <a href="{{ asset('storage/'.$payment->payment_proof) }}" target="_blank" class="inline-flex items-center px-3 py-1 text-sm font-medium text-blue-700 bg-blue-100 border border-blue-200 rounded-lg hover:bg-blue-200 transition duration-150">
                        <i class="fas fa-eye mr-2"></i> Lihat Bukti Transfer
                    </a>
                    <div class="flex items-center pt-1">
                        <input class="w-4 h-4 text-red-600 border-gray-300 rounded focus:ring-red-500" type="checkbox" id="remove_proof" name="remove_proof">
                        <label class="ml-2 text-sm text-gray-600" for="remove_proof">
                ...
                <p class="mt-1 text-xs text-gray-500">Format yang didukung: JPG, PNG, PDF. Maksimal 2MB. (Abaikan jika tidak ingin mengubah)</p>
                @error('payment_proof')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-4 pt-4 border-t">
                <a href="{{ route('admin.payments.index') }}" class="px-4 py-2 text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-100 transition duration-150 flex items-center space-x-2">
...
<script>
    ...
    // Handle checkbox untuk menghapus bukti transfer
    document.getElementById('payment_proof').addEventListener('change', function() {
        if (this.files.length > 0) {
            document.getElementById('remove_proof').checked = false;
        }
    });

    document.getElementById('remove_proof').addEventListener('change', function() {
        if (this.checked) {
